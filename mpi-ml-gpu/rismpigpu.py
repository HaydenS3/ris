...
if rank == 0 and best_rank == 0:
    best_model.save(f'best_cnn_model_gpu_{datetime.datetime.now().strftime("%Y%m%d_%H%M%S")}.keras')
elif rank != 0 and should_save:
    best_model.save(f'best_cnn_model_gpu_{rank}_{datetime.datetime.now().strftime("%Y%m%d_%H%M%S")}.keras')
